...
public class Move {
	...
	public static int connectingDots(int boardHeight, int boardWidth, List<SnakeInfo> otherSnakes, SnakeInfo mySnake,
			Location from, Location current, Location start, Set<Location> connectedLocation, boolean topBottomRule) {
		...
		if (!left.equals(from) && !left.equals(start)) {
			if (left.isEmpty(boardHeight, boardWidth, otherSnakes, mySnake, topBottomRule, false)) {
				if (!connectedLocation.contains(left)) {
					...
					connectionPoints = connectionPoints + connectingDots(boardHeight, boardWidth, otherSnakes, mySnake,
							current, left, start, connectedLocation, topBottomRule);
				}
			}
		}
		if (!right.equals(from) && !right.equals(start)) {
			if (right.isEmpty(boardHeight, boardWidth, otherSnakes, mySnake, topBottomRule, false)) {
				if (!connectedLocation.contains(right)) {
					connectedLocation.add(right);
					connectionPoints = connectionPoints + 1;
					connectionPoints = connectionPoints + connectingDots(boardHeight, boardWidth, otherSnakes, mySnake,
							current, right, start, connectedLocation, topBottomRule);
				}
			}
		}
		if (!up.equals(from) && !up.equals(start)) {
			if (up.isEmpty(boardHeight, boardWidth, otherSnakes, mySnake, topBottomRule, false)) {
				if (!connectedLocation.contains(up)) {
					connectedLocation.add(up);
					connectionPoints = connectionPoints + 1;
					connectionPoints = connectionPoints + connectingDots(boardHeight, boardWidth, otherSnakes, mySnake,
							current, up, start, connectedLocation, topBottomRule);
				}
			}
		}
		if (!down.equals(from) && !down.equals(start)) {
			if (down.isEmpty(boardHeight, boardWidth, otherSnakes, mySnake, topBottomRule, false)) {
				if (!connectedLocation.contains(down)) {
					connectedLocation.add(down);
					connectionPoints = connectionPoints + 1;
					connectionPoints = connectionPoints + connectingDots(boardHeight, boardWidth, otherSnakes, mySnake,
							current, down, start, connectedLocation, topBottomRule);
				}
			}
		}
